...
/*	case 'credit_add':
	...
	case 'deletechess':{
		...
		$list_all = pdo_fetchall("SELECT * FROM " . tablename($this->table_collect) . " WHERE uniacid={$_W['uniacid']}");
		foreach($list_all as $list){
			$condition['filename'] = $list['filename'];
			if(!pdo_get($this->table_chess, $condition)){
				pdo_delete($this->table_collect, $condition);
			}
		}
	}*/
