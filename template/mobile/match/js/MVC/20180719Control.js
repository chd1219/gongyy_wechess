...
/*检查是否被将军*/
comm.Checked = function () {
	try{
		if(waitServerPlay){
			setTimeout((function(){comm.Checked()}),500);
			return;
		}
		
		my = comm.getHold();
		my == -1 ? jiang = comm.mans["j0"] : jiang = comm.mans["J0"];
...
comm.DealPosition = function (obj) {
	if(obj.result != null && obj.result != undefined){
		...
		if(d.match("checkmate"))	{
			comm.getHold()==RED ? showFloatTip("红方被将死！！"):showFloatTip("黑方被将死！！");		
			/*回调返回函数*/
			myws.Return();
			comm.onGameEnd();		
			return;
